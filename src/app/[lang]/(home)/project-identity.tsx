...
export function BrandUnderline(props: { tone: ProjectTone; children: ReactNode }) {
  ...
  return (
    <span
      className="underline decoration-[2.5px] underline-offset-[0.18em]"
      style={{ textDecorationColor: projectTones[tone].accent }}
    >
      {children}
    </span>
  );
...
export function HighlightedName(props: { text: string; name: string; tone: ProjectTone }) {
  const { text, name, tone } = props;
  ...
  return (
    <>
      {parts.map((part, index) => (
        <span key={`${part}-${index}`}>
          {part}
          {index < parts.length - 1 ? <BrandUnderline tone={tone}>{name}</BrandUnderline> : null}
        </span>
      ))}
    </>
  );
...
export function ProductLogo(props: {
  ...
  return (
    <span
      aria-hidden="true"
      className={cn(
        'inline-flex size-11 shrink-0 items-center justify-center rounded-lg border border-black/10 text-fd-muted-foreground dark:border-white/10',
        className,
      )}
    >
      {logo === 'narraleaf' ? (
