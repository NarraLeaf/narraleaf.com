...
NarraLeaf is not only a single library. It is a project structure for keeping visual novel work understandable as it grows.
...
This lets a project begin with a visual workflow, move deeper into engine and delivery concerns when needed, or embed only the player layer inside an existing React product.
...
[NarraLeaf](/docs/narraleaf/library) is the desktop application toolchain. It covers the runtime, renderer, and CLI used to build and deliver NarraLeaf applications.
...
This path keeps the player close to the React ecosystem, making it useful when NarraLeaf is one part of a larger application.
...
Start with [NarraLeaf](/docs/narraleaf/library) if you are building a complete desktop visual novel application and need access to the runtime, renderer, CLI, and delivery workflow.
