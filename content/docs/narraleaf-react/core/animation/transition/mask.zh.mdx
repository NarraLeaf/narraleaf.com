---
title: "遮罩图案"
description: Reveal 与 ThroughColor 使用的遮罩图案：扫描、光圈、时钟等
---
...
每种图案都有自然方向和反转（互补）方向，即同一几何形状从另一侧覆盖：擦除从对侧推进，圈显从四周向中心收拢而不是从中心展开，时钟指针向另一方向扫掠。引擎通过 `ThroughColor` 的 `inverted` 选项及其 `"continue"` 揭开方式使用反转方向；也可以用 [`Mask.invert`](#maskinvert) 永久翻转
