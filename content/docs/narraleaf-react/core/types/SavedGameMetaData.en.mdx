...
The save format version. `2` for saves that carry the persisted backlog (see [SavedGame](/docs/narraleaf-react/core/types/SavedGame)'s `history`); absent on legacy saves written before it existed, which load unchanged with an empty backlog.
