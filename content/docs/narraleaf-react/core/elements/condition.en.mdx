---
title: "Condition"
description: The `Condition` class for branching a story with `If`, `ElseIf`, and `Else` blocks.
---
...
In usual cases, you may want to determine flags, or check the value of a variable. You can use [Perdidtent](/docs/narraleaf-react/core/elements/persistent) to do this.
...
- `condition: Lambda | LambdaHandler<boolean>` - The condition to check. If the condition is true, the actions will be executed. See [Lambda](/docs/narraleaf-react/core/types/Lambda) and [LambdaHandler](/docs/narraleaf-react/core/types/LambdaHandler) for more information.
- `action: ActionStatements` - [ActionStatements](/docs/narraleaf-react/core/types/ActionStatements)
...
- `config?: ConditionConfig` - [ConditionConfig](/docs/narraleaf-react/core/types/ConditionConfig)
</Step>
</Steps>
