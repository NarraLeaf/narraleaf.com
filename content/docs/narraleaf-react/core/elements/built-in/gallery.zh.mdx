---
title: "Gallery"
description: 内置的 `Gallery` 服务，管理图鉴物品的增删查询，支持存档或本地存储持久化
---
Gallery 是专门用于视觉小说游戏画廊（图鉴）的内置服务，可以添加、删除和检查项目。Gallery 数据可以独立持久化到 **localStorage**，也可以随 **游戏存档** 通过 `serialize`/`deserialize` 自动保存

Gallery 适合用于管理游戏中的图片收集系统，比如角色立绘、CG 图片、背景图片等
...
要使用 Gallery，需要在故事中注册它：

```typescript
story.registerService("gallery", gallery);
```

注册后，就可以通过游戏上下文访问 Gallery：
...
可将 Gallery 数据独立于游戏存档持久化到 [localStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage)。调用 `serialize()` 获取数据，加载时调用 `deserialize()`：

```typescript
const GALLERY_STORAGE_KEY = "my-game-gallery";

// 保存到 localStorage（例如打开图鉴页时或定时保存）
...
// 从 localStorage 加载（例如游戏初始化时、newGame 之前）
...
这样即使玩家从未使用完整存档系统，图鉴解锁状态也能跨会话保留

## serialize / deserialize 与游戏存档

Gallery 实现了 `serialize()` 和 `deserialize()`。当使用 [LiveGame.serialize](/zh/docs/narraleaf-react/core/game/live-game#serialize) 保存游戏时，框架会自动将 Gallery 的序列化数据写入 `SavedGame.game.services`。当调用 [LiveGame.deserialize](/zh/docs/narraleaf-react/core/game/live-game#deserialize) 加载存档时，框架会恢复所有已注册服务，包括 Gallery

因此，若使用将 `liveGame.serialize()` 存入 localStorage 的 [存档系统](/zh/docs/narraleaf-react/solutions/save-system-localstorage)，图鉴解锁状态会随游戏 **自动** 保存和恢复，无需额外代码。`serialize`/`deserialize` 方法正是实现这一集成的关键

## 链式方法

这些方法返回动作，需要放进 `scene.action` 中使用，也可以链式调用
...
- 返回 `Lambda<boolean>` - 项目存在时返回 `true`，否则返回 `false`
</Step>
</Steps>
...
- `name: string` - 要检查的项目名称
- 返回 `boolean` - 项目存在时返回 `true`，否则返回 `false`
</Step>
</Steps>
