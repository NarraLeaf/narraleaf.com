...
Elements reachable from a scene's action tree receive generated ids (`e-0`, `e-1`, ...) at story construction. An element registered directly via [registerDisplayable](#registerdisplayable) is outside that tree and would otherwise keep the default id, colliding with others as a React key — so give it a unique id, with a distinct prefix so it never collides with a generated one.
