---
title: "Vfx"
description: The `Vfx` element for full-screen looping video overlays such as rain, snow, or petals, with preload, fade, and playback-rate control.
---
...
- `show`/`hide` fades **complete instantly** when the player skips, and a source that fails to load logs an error and resolves immediately, so a broken asset never blocks the story.
- Overlays on the stage are captured by save/load and re-appear without a fade, playing or frozen if paused. Since `0.33.0` this covers preloaded overlays as well, which restore hidden. Saves created before `0.16.0` load fine.
- A save naming an overlay the story no longer has loads without it, and logs a warning.
...
petals.show({opacity: 0.35, rate: 2});   // faint and fast, this time only
```
</Step>
</Steps>
