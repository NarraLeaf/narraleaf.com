---
title: "Service"
description: The `Service` abstract class for custom, story-registered logic shared across scenes, covering actions, serialization, and async handling.
---
...
For ServiceHandlerCtx, see [ServiceHandlerCtx](/docs/narraleaf-react/core/types/ServiceHandlerCtx).
...
When the player performs an _undo_ while this service is still running, the game engine will automatically call every `onAbort` callback registered in the current action context. It is **script's responsibility** to cancel any in-flight tasks (network requests, timers, animations, etc.) and roll back side-effects created so far. Doing so guarantees that if the same action is executed again (after the undo) it will yield exactly the same result.
...
To trigger an action, you can use the `trigger` method.
...
- `handler: ServiceHandler<Content[K]>` - For ServiceHandler, see [ServiceHandlerCtx](/docs/narraleaf-react/core/types/ServiceHandlerCtx)
- Returns `this`
</Step>
</Steps>
