...
如果脚本内容包含 Persistent，则必须使用 [Story](/zh/docs/narraleaf-react/core/elements/story) 进行注册
...
请注意命名空间的**键**：名为 `"persis"` 的 `Persistent` 注册时的键是 `"persistent:persis"`。请在 `Storable` 而不是 [Namespace](/zh/docs/narraleaf-react/core/game/storable/namespace) 上订阅，并把 `onChange` 与 `onRestore` 搭配使用，以免漏掉读入的存档——参见[监听存储值的变化](/zh/docs/narraleaf-react/core/game/storable/events)。
