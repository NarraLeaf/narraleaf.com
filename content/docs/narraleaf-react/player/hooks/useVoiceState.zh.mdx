---
title: "useVoiceState"
description: useVoiceState 钩子，跟踪当前句子自动与手动重播的语音播放状态
---
...
- `playVoice(voice?: Sound | string | URL | null): Promise<SoundToken | null>` — 手动播放语音，传入字符串或 URL 会自动创建一个 `Sound` 实例，传入 `null` 可停止手动语音。此函数最终调用 [`LiveGame.playSound`](/zh/docs/narraleaf-react/core/game/live-game) 并返回 `SoundToken`，可以用来监听 `ended` 或 `stop` 事件
