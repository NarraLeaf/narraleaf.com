...
- `autoFit?: boolean` - Fits the whole line inside its box by setting it smaller. Default `false`. Since `0.34.0`.
- `autoFitMinFontSize?: number` - Smallest size `autoFit` sets, in px. Default `12`. Since `0.34.0`.
...
`autoFit` turns `fontSize` into a ceiling. The line is set at `fontSize` whenever the finished line fits the box, and smaller when it does not, down to `autoFitMinFontSize`. A line that still overflows at the smallest size is left overflowing.

```tsx
<Texts fontSize={24} autoFit autoFitMinFontSize={16} />
```

The finished line is what has to fit, so the size is settled before the first character appears and holds for the rest of the line.

Sizes set on a `Sentence` or a `Word` are multiplied by the same factor, so their relative weights hold at every size.
