...
Exported prop types: `NametagProps`, `TextAppearanceProps`, `TextsProps`, `RawTextsProps`, and `EntryTextsProps`.

### Nametag

Renders the speaker name. Must be used inside `Dialog`.

`Nametag` is now component-driven: pass props to control its content and presentation. If no custom content is provided, it falls back to the current speaker name. If no `color` prop is provided, it falls back to the current character color when available.

- `character?: Character | null` — Explicit character source.
- `entry?: NvlDialogEntry` — NVL entry source.
- `name?: React.ReactNode` — Override the displayed name.
- `color?: Color` — Text color for the nametag.
- `children?: React.ReactNode` — Fully custom nametag content.
...
Text defaults are component props or inherited CSS. `Sentence` and `Word` style overrides still win for the individual line or word.

- `children?: never`
- `defaultColor?: Color` — Fallback text color when sentence/word color is not set.
- `fontSize?: React.CSSProperties["fontSize"]`
- `fontWeight?: React.CSSProperties["fontWeight"]`
- `fontWeightBold?: React.CSSProperties["fontWeight"]` — Weight used when a sentence/word is bold.
