...
- `autoFit?: boolean` - 缩小字号，使整行放进它所在的盒子。默认 `false`。自 `0.34.0` 起
- `autoFitMinFontSize?: number` - `autoFit` 能设到的最小字号，单位 px。默认 `12`。自 `0.34.0` 起
...
`autoFit` 把 `fontSize` 变成上限。整行放得下时按 `fontSize` 排版，放不下时缩小，最小到 `autoFitMinFontSize`。在最小字号下仍然溢出的行保持溢出

```tsx
<Texts fontSize={24} autoFit autoFitMinFontSize={16} />
```

要放得下的是完整的一行，因此字号在第一个字出现之前就已确定，并在该行逐字显示的全程保持不变

`Sentence` 与 `Word` 上设定的字号按同一系数一并缩放，相对大小关系在任何字号下都成立
