---
title: "Show Image"
description: Constructing an Image element, showing and hiding it on stage, and animating its opacity with a transition.
---
...
NarraLeaf 0.30 includes the `Dissolve`, `FadeIn`, `BlurDissolve`, `Push`, `Darkness`, `Reveal`, `RuleReveal`, `ThroughColor`, and `Exposure` engines, plus the `Mask` pattern vocabulary (wipe, barn door, iris, clock, fan, blinds, dots) that `Reveal` and `ThroughColor` animate. All image transitions work with static, tag-based, and layered images.
