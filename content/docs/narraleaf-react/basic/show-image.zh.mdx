...
NarraLeaf 0.30 内置 `Dissolve`、`FadeIn`、`BlurDissolve`、`Push`、`Reveal`、`RuleReveal`、`ThroughColor` 和 `Exposure` 这些转场方式，以及供 `Reveal` 和 `ThroughColor` 播放的 `Mask` 图案词汇（擦除、对开门、圈显、时钟、风车、百叶窗、圆点）。这些过渡都支持普通图片、标签图片和层叠图片
