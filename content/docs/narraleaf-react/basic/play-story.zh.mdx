---
title: "播放故事"
description: 设置 Story 的入口场景，在 GameProviders 中渲染，并在 Player 的 onReady 回调里开始播放
---
...
注意 `onPreloadComplete` 是*另一个*时点：自 `0.17.0` 起，入口场景在故事被加载时就已开始预热，因此该回调在进入游戏之前就会触发，此时菜单可能还在屏幕上。详见[预加载](/zh/docs/narraleaf-react/core/game/preload#该等哪个信号)
