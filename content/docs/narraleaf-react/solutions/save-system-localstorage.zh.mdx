---
title: "localStorage 存档系统（含快捷保存）"
---
## 概述

NarraLeaf-React 不内置存档 UI，但提供 [serialize](/zh/docs/narraleaf-react/core/game/live-game#serialize) 和 [deserialize](/zh/docs/narraleaf-react/core/game/live-game#deserialize) 方法用于序列化/反序列化游戏状态。结合 [localStorage](https://developer.mozilla.org/en-US/docs/Web/API/Window/localStorage) 可实现简单的浏览器端存档，并配合 [KeyMap](/zh/docs/narraleaf-react/core/game/key-map) 实现快捷保存（如 Ctrl+S）和快捷读取（如 F9）。

本文档演示多槽位存档与快捷保存/读取的完整实现。

## 1. 保存与读取核心逻辑

使用 [useLiveGame](/zh/docs/narraleaf-react/player/hooks/useLiveGame) 或 `game.getLiveGame()` 获取 [LiveGame](/zh/docs/narraleaf-react/core/game/live-game)，调用 `serialize()` 得到 [SavedGame](/zh/docs/narraleaf-react/core/types/SavedGame)，再存入 localStorage：
...
在存档/读档页面中列出各槽位，显示 [SavedGameMetaData](/zh/docs/narraleaf-react/core/types/SavedGameMetaData) 中的信息：
...
- **脚本兼容性**：修改故事脚本后，旧存档可能无法正确加载，[LiveGame](/zh/docs/narraleaf-react/core/game/live-game) 文档中有说明。
- **存储限制**：localStorage 约 5–10MB，存档过大时需考虑 [IndexedDB](https://developer.mozilla.org/en-US/docs/Web/API/IndexedDB_API) 或服务端存储。
- **Storable 与 Service**：[Storable](/zh/docs/narraleaf-react/core/game/storable) 和已注册的 [Service](/zh/docs/narraleaf-react/core/types/ServiceHandlerCtx)（如内置 [Gallery](/zh/docs/narraleaf-react/core/elements/built-in/gallery)）会随 `serialize`/`deserialize` 一并保存和恢复。

## 参考

- [LiveGame.serialize / deserialize](/zh/docs/narraleaf-react/core/game/live-game) - 序列化与反序列化
- [SavedGame](/zh/docs/narraleaf-react/core/types/SavedGame) - 存档数据结构
- [KeyMap](/zh/docs/narraleaf-react/core/game/key-map) - 按键绑定
