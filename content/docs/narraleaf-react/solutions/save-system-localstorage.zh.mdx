...
description: 基于 serialize 与 deserialize，在 localStorage 中实现带校验的五槽位存档系统
---
...
自存档格式 v3（`0.26.0`）起，写出的元素状态只包含与剧本所写不同的部分，读档会在应用存档之前重置每一个元素 —— 因此存档的大小取决于舞台上有什么，而不是项目有多大。0.26.0 写出的存档无法被更旧的引擎正确读取，参见 [SavedGame](/zh/docs/narraleaf-react/core/types/SavedGame)
