---
title: "Custom Menu (In-Game Choices)"
description: Replacing the in-game choice menu with GameMenu and Item, including keyboard shortcuts through Item's bindKey prop.
---
## What It Replaces

The in-game Menu displays choice branches during the story; the player selects an option by click or keyboard shortcut to trigger the corresponding action. You can replace the default [Menu](/docs/narraleaf-react/player/menu) component via the `menu` option in [game.configure](/docs/narraleaf-react/core/game/game) to customize the choice appearance and layout.

Use [GameMenu](/docs/narraleaf-react/player/menu) as the choice container and [Item](/docs/narraleaf-react/player/menu) for each choice. `Item.bindKey` adds a keyboard shortcut.

## 1. Create the Custom Menu Component

The custom menu component receives `items` (an array of choice indices), uses [GameMenu](/docs/narraleaf-react/player/menu) as the container, and [Item](/docs/narraleaf-react/player/menu) to render each option. `Item` automatically gets the text and click handler from context.
...
Menu choice text is controlled by the custom menu component. Pass `defaultColor`, `fontSize`, `fontFamily`, `fontWeight`, or `fontWeightBold` to `Item`, or let the text inherit from CSS.
...
Choices are defined in the [Menu](/docs/narraleaf-react/core/elements/menu) element via `choose`, with support for `hideIf`, `disableIf`, etc.:
...
- [Menu](/docs/narraleaf-react/player/menu) - Menu component API
- [Menu element](/docs/narraleaf-react/core/elements/menu) - Menu usage in scripts
