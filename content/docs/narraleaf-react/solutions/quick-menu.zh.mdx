...
description: 整合回退、重做、历史、自动前进、存读档、设置与退出的快捷菜单面板
---
## 范围
...
<Callout type="info">
    `undo()` 与 `redo()` 在无处可去时返回 `false`，因此上面的按钮在时间线的任一端按下都是安全的。若想改为把它们置灰，请使用 [`canUndo()`](/zh/docs/narraleaf-react/core/game/live-game#canundo) / [`canRedo()`](/zh/docs/narraleaf-react/core/game/live-game#canredo)
</Callout>
