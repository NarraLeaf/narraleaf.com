---
title: "自定义对话框"
description: 用 Nametag、Texts 与 useDialog 替换默认 Dialog 组件，自定义布局与状态读取
---
## 作用
...
下面使用 [Nametag](/zh/docs/narraleaf-react/player/dialog) 和 [Texts](/zh/docs/narraleaf-react/player/dialog) 分别渲染角色名和台词
...
创建 [Game](/zh/docs/narraleaf-react/core/game/game) 时指定 Dialog，再把同一个实例传给 `GameProviders`。把 Game 放在 React 组件外，也可以避免每次渲染都重新创建实例
...
这个版本用 [useDialog](/zh/docs/narraleaf-react/player/hooks/useDialog) 的 `done` 控制打字完成标记，用 `isNarrator` 控制名牌显隐：
