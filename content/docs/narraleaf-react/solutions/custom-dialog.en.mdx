---
title: "Custom Dialog"
description: Replacing the default Dialog component with Nametag, Texts, and the useDialog hook for custom layout and state.
---
## What It Replaces
...
The implementation below uses [Nametag](/docs/narraleaf-react/player/dialog) and [Texts](/docs/narraleaf-react/player/dialog) for the character name and line content.
...
Set the dialog component when creating [Game](/docs/narraleaf-react/core/game/game), then pass that instance to `GameProviders`. Creating the game outside the React component also avoids rebuilding it on every render.
...
- **Nametag content and color**: Controlled by `Nametag` props such as `name`, `children`, `color`, `className`, and `style`.
- **Text defaults**: Controlled by `Texts` props such as `defaultColor`, `fontSize`, `fontFamily`, `fontWeight`, and `fontWeightBold`. [Word](/docs/narraleaf-react/core/elements/character/word) / [Sentence](/docs/narraleaf-react/core/elements/character/sentence) styling still overrides specific words or lines.
