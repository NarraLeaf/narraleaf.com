---
title: "Quick Menu"
description: A quick-access panel for undo, redo, history, auto-forward, save, load, settings, and exit in one component.
---
## Scope
...
<Callout type="info">
    `undo()` and `redo()` return `false` when there is nowhere to go, so the buttons above are safe
    to press at either end of the timeline. Use
    [`canUndo()`](/docs/narraleaf-react/core/game/live-game#canundo) /
    [`canRedo()`](/docs/narraleaf-react/core/game/live-game#canredo) to disable them instead.
</Callout>
