...
description: A concept map from Ren'Py labels, jumps, and screens to NarraLeaf's Scene, Character, and Menu equivalents.
---
...
- There is no rollback UI by default, though [`liveGame.undo()`](/docs/narraleaf-react/core/game/live-game#undo) and [`redo()`](/docs/narraleaf-react/core/game/live-game#redo) provide the underlying actions, with `canUndo()` / `canRedo()` to gate the buttons. Like Ren'Py's rollback, stepping back does not throw away what is ahead: reading forward again retraces the same lines, and the future is dropped only when the story goes somewhere else.
