...
store 不属于任何存档槽位，也不是编辑器侧的 [`app.services.storage`](/docs/studio/plugin/studio/storage)——后者只在工作区打开时存在。如果你需要把在 Studio 里编写的数据送到游戏中，请用 [`contributes.runtimeData`](/docs/studio/plugin/manifest#runtimedata) 一起发布，并改用 `app.game.data.readJson` 读取——那是只读的，并随包一同分发。
