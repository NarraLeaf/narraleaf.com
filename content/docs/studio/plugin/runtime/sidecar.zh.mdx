...
`include` 列出为该平台分发的一切。条目要么是相对于包的路径，要么是 `dep:<buildDependencyId>/<path>`，用来取出某个已声明的[构建时依赖](/docs/studio/plugin/build-dependencies)产出的产物——你自己可能无权镜像的可再分发文件就是这样进包的。`entry` 也必须出现在 `include` 里。
