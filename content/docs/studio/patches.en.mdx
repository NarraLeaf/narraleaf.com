...
Zip the `patch` folder and send it. The player extracts it into the game's folder:
...
The game's folder is the one holding its `resources` folder. On Windows and Linux the executable is in that folder. On macOS it is `Contents`, inside the app bundle: right-click the app, choose **Show Package Contents**, and open `Contents`.

Deleting the `.patch.dat` file returns the game to what it shipped with.

A `patch` folder in the player data folder also applies. A patch kept there survives reinstalling the game, and wins over one in the game's folder. **Project ▸ App** lists that folder's path on each platform under **Player files**.
