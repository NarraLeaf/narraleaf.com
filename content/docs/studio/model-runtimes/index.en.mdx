...
Studio is free software with published source, and the licences these renderers ship under do not permit that combination: Live2D's forbids letting a source-available, freely modifiable licence cover its code, and Spine's requires whoever integrates a runtime to hold a paid Editor licence. NarraLeaf therefore cannot distribute either renderer with Studio, and fetching one on the user's behalf would also constitute distribution.
...
A runtime is project content, not a cache. It belongs in version control alongside the assets, and it is copied into every build. The Live2D runtime file is around 1 MB, because the renderer is compiled into it.
