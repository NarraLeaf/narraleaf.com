...
The dialog states what is being agreed to and links to Live2D's download page. Studio does not fetch the SDK, because the download page is where Live2D's licence is accepted. Tick the acknowledgement to continue.
...
`index.js` is generated and about a megabyte, with the renderer compiled into it. `licenses/` holds Live2D's own licence texts exactly as they shipped in the archive, and `README.md` records which SDK version produced the file. All of it is project content: it belongs in version control, and every build carries it.
...
| Symptom | Usual cause |
| --- | --- |
| "Runtime not installed in this project" | The runtime is not installed on this machine. A fresh clone, or a project that never had it. |
