...
<Callout type="warn">
    **Spine Runtimes are licensed by Esoteric Software.** Their Editor License Agreement requires whoever integrates a Spine runtime into a tool to hold a valid Spine Editor licence; a trial licence does not satisfy it. NarraLeaf holds no Spine licence, so Studio ships no Spine code and offers no build-it-for-me step.
</Callout>
...
2. **Integration is performed by the user**, by building the adapter described below. The Editor licence covers that.
3. **The Spine Runtimes License must be supplied with the game that ships**, in full, in the documentation distributed with the product. That obligation belongs to whoever ships the game.
...
await build({
    ...
    platform: "browser",
    target: "es2022",
    // Keeps the Spine Runtimes licence header in the output.
