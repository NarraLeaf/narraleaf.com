...
A blueprint is not the only way to fill a slot. The same logic can be written as a **script** instead — a TypeScript file the project owns, edited outside Studio — and the two are revisions of the same slot, of which only the active one runs. [Scripts](/docs/studio/scripts) covers where those files live and how one is entered.
