...
- **The project has to allow it.** [Network access](/docs/studio/security#network-access) is **Off** in every new project, and while it is off these nodes cannot run. Either of the other two positions — Allowlist or Any host — lets them run, and an Allowlist also decides which addresses they may reach. See [When the project does not allow HTTP](#when-the-project-does-not-allow-http) below.
...
With Network access set to Off, the shipped game is confined to its own protocol and every HTTP request is cancelled, so a network node in the graph is code that cannot run. Three things say so, at falling distance from the author:
...
The remedy is either one: move Network access off Off — to Allowlist, naming the addresses, or to Any host — or remove the nodes.
...
`networkError` is the catch-all for "no HTTP response happened": DNS failure, connection refused, a scheme that is not http(s), a response over the size cap, a project whose Network access is Off, and an address an Allowlist does not name. They share a pin because an author's response to all of them is the same — there is no data, show the player something else — and the `Error` pin says which it was.
...
**Headers are a JSON object**, most easily built with `Make Object`. Values that are numbers or booleans are sent as their text; nested objects and arrays are dropped, since there is no header they could correctly become.
