...
`Go Page` with a target is the only way to open a layer. To close one, prefer [Go back](#go-back): navigation is a stack, so `Go Page` at a page the player came *from* pushes a third page over the two already there rather than returning to it. `Go Page` with `None` closes the current top Page layer.
