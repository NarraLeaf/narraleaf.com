...
A `function` graph is not the body of a fn — that is an ordinary event graph, and its head is `Fn` in [Functions](/docs/studio/blueprint/nodes/functions). `function` is the graph kind that backs a computed value, and it is offered only pure, non-latent nodes.
