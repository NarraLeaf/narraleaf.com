...
这些节点用的是通用 `element` 引脚，因此接受任意可显示元素的引用，而不限定某一种控件类型。坐标使用当前 Surface 的设计坐标系——和 Inspector 里显示的是同一套数。透明度只有一份有效值：Appearance Variant 的 `transformOpacity` 会解析到它，`nl.image` Variant 中相对 Default 改动过的 `fillOpacity` 也会解析到它，`Get Element Property`、`Set Element Property` 和 `Animate Element Property` 操作的都是这同一个数。
