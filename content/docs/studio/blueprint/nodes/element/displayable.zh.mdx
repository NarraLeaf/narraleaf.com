...
两组适用于所有控件类型的节点：Displayable 的几何与外观节点，以及每个内建控件都重复一遍的 `visible` / `enabled` 四件套。所有 Element 节点共通的规则——创建浮窗何时提供它、目标缺失时如何失败——见 [Element](/docs/studio/blueprint/nodes/element)。
...
它们全部的 Self 形态见 [Displayable](/docs/studio/blueprint/nodes/displayable)。
