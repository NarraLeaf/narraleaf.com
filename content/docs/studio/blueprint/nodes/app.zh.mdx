...
App 节点是蓝图伸手够到它所运行的那个应用的地方：打开一个 Surface、关掉最上面那个、进入全屏、退出应用。Surface 模型本身——栈、层叠、游戏舞台——在 [Surfaces](/docs/studio/blueprint/surfaces) 中说明。
...
游戏一旦跑起来，Page 栈会被藏到舞台下面。此时 `Go Page` 会把目标作为 UI 叠层打开在舞台*之上*，`None` 则关闭该叠层、重新露出舞台。要离开游戏本身，请用 [Game](/docs/studio/blueprint/nodes/game) 里的 `Quit Game`——它自己指定要返回的 Page。
...
要取出其中某个字段，用 [Data](/docs/studio/blueprint/nodes/data) 里的 `Get JSON Field`，它接受点分路径。
...
Emit Page Event（触发页面事件）把事件从被嵌入的 Page 向上发给嵌入它的那个 `nl.frame` 元素。Frame 自己的蓝图通过 [Events](/docs/studio/blueprint/nodes/events) 里的 `Page Event` 事件 Head 接收，并原样拿到同一个名字和载荷。
