...
The two nodes that used to steer that, `Continue Event Bubble` and `Stop Event Bubble`, were removed. The one thing they were reliably used for — holding a window close open — is now [`Keep Window Open`](/docs/studio/blueprint/nodes/app) in the App category, which does that and nothing else.
