...
To ask whether an action is *being held* rather than react to it being raised, use `Is Action Held` in [Input](/docs/studio/blueprint/nodes/input).
...
Three heads on `nl.switch`, and all three are about the player toggling it. Writing the state from a graph — `Set Checked`, `Turn On`, `Turn Off`, `Toggle` in [Switch](/docs/studio/blueprint/nodes/switch) — does not dispatch any of them, so a graph that writes the switch it also listens to does not trigger itself.
