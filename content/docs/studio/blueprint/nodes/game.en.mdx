...
Reads back exactly what `Save Game` put on its `metadata` pin. System fields — the id, the record type, `createdAt`, `updatedAt`, the preview — are not part of it. `Get Save Time` reads the two timestamps.
...
When a slot was written. The save store stamps every record it writes, and this reads those stamps back — a save screen does not have to put a time into its own `metadata` and keep it in step.
...
Both times are milliseconds since 1 January 1970 UTC — the same unit `List Auto Saves` reports and every [Time](/docs/studio/blueprint/nodes/time) node accepts.

<Callout type="warn">
Read `exists` to tell an empty slot apart. A missing save gives `savedAt` `0`, and `0` is also a real moment in 1970.
</Callout>
