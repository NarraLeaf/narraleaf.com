...
- **A host API is not a playthrough.** `List Saves`, `Get Save Metadata`, `Get Save Preview`, `Delete Save`, `List Auto Saves` and `Get Latest Auto Save` read the project's save store and work between playthroughs, with no game running. Everything that serializes, deserializes or commands the live game needs one, and fails without it.
...
Reads back exactly what `Save Game` put on its `metadata` pin. System fields — the id, the record type, `createdAt`, `updatedAt`, the preview — are not part of it.
