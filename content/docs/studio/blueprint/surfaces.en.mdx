...
- **Opening pushes.** `Go Page` pushes the selected Page with whatever you wired into its `props` input.
- **Closing pops the top layer.** `Go Page` with its Page selector set to `None` *is* the close: an empty target closes the top layer instead of opening anything. There is no separate back node.
- **The floor is permanent.** A close is refused when only one layer is left, so the entry Surface is always underneath something. Signatures are on [App nodes](/docs/studio/blueprint/nodes/app).
...
Its event heads: `Surface Init`, `Surface Unmount`, `Before Surface Exit`, `After Surface Enter`, `Mouse Click`, `Right Click`, the keyboard heads, the game-preference heads, fullscreen changes, the window-close request, and the broadcast receivers.
